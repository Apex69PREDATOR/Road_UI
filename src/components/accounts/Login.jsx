...
const Login = () => {
  ...
  const onsubmit=async(data)=>{
    submitting(true)
     const res=await fetch(`${import.meta.env.VITE_SERVER_URL}/authenticate/login`,{method:"POST",headers:{
  ...
  return (
    <div className='relative bg-black flex justify-center items-center w-[101%] h-[100vh] border-2 border-black'>
        ...
        <button className='bg-[#00e900]  text-black h-[10%] w-[60%] rounded'>{submit?'Signing In...':'Sign In'}</button>
        </form>
    </div>
  )
}
