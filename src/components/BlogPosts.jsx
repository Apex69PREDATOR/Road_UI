  ...
  return (
    ...
    <p><i onClick={()=>{showComment(prev=>({...prev,[val._id]:!comment[val._id]}))}} className='fa-regular fa-comment text-2xl cursor-pointer hover:scale-125 transition-transform duration-200'></i>
          ...
          </div>
          {
            ...
            }):<p style={{opacity:"0.5"}}>No comments till now</p>} <input className='absolute w-[70%] border-b-2 border-[rgb(0,0,0,0.6)] bottom-[2%]' ref={cmtbox} type="text" onKeyDown={(e)=>{e.target.key=='Enter' && give_comment()}} placeholder='Comment something..' /> </div>
           }
          </>
