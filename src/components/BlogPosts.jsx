    ...
    <p><i onClick={()=>{showComment(prev=>({...prev,[val._id]:!comment[val._id]}))}} className='fa-regular fa-comment text-2xl cursor-pointer hover:scale-125 transition-transform duration-200'></i>
          ...
          </div>
          {
            comment[val._id] && <div className='comment flex flex-col justify-start items-start flex-wrap gap-[5%] [&_*]:ml-[5%] relative bg-white rounded h-[30vh] w-[90%] md:w-[60%]'>{commentarr[val._id].length?commentarr[val._id].map(val=>{
              return <div id={val.uid} className='flex bg-[#f7f7f7] mt-[1%] rounded p-2 flex-col h-[20%] w-[80%]'><p className='font-semibold text-[0.75em]'>{val.name}</p><p className='p-1'>{val.comment}</p></div>
